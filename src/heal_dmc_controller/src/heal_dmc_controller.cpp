...
#include "heal_dmc_controller/heal_dmc_controller.h"

HealDMCController::HealDMCController()
  :node_handle_(""),
   priv_node_handle_("~"),
...
bool HealDMCController::initWorkbench(const std::string port_name, const uint32_t baud_rate)
{
  bool result = false;
  const char* log;
...
bool HealDMCController::getDynamixelsInfo(const std::string yaml_file)
{
  YAML::Node dynamixel;
...
bool HealDMCController::loadDynamixels(void)
{
  bool result = false;
  const char* log;
...
bool HealDMCController::initDynamixels(void)
{
  const char* log;
...
bool HealDMCController::initControlItems(void)
{
  bool result = false;
  const char* log = NULL;
...
bool HealDMCController::initSDKHandlers(void)
{
  bool result = false;
  const char* log = NULL;
...
bool HealDMCController::getPresentPosition(std::vector<std::string> dxl_name)
{
  bool result = false;
  const char* log = NULL;
...
void HealDMCController::initPublisher()
{
...
void HealDMCController::initServer()
{
  dynamixel_command_server_ = priv_node_handle_.advertiseService("dynamixel_command", &HealDMCController::dynamixelCommandMsgCallback, this);
}

void HealDMCController::readCallback(const ros::TimerEvent&)
{
#ifdef DEBUG
...
void HealDMCController::publishCallback(const ros::TimerEvent&)
{
#ifdef DEBUG
...
// NEW 21/01 TODO
void HealDMCController::sinPositionCallback(const std_msgs::Float64::ConstPtr &msg) 
{
  bool result = false;
  const char* log = NULL;
  double i;
...
bool HealDMCController::dynamixelCommandMsgCallback(dynamixel_workbench_msgs::DynamixelCommand::Request &req,
...
int main(int argc, char **argv)
{
  ...
  HealDMCController dynamixel_controller;
  ...
  ros::Timer read_timer = node_handle.createTimer(ros::Duration(dynamixel_controller.getReadPeriod()), &HealDMCController::readCallback, &dynamixel_controller);
  ros::Timer write_timer = node_handle.createTimer(ros::Duration(dynamixel_controller.getWritePeriod()), &HealDMCController::writeCallback, &dynamixel_controller);
  ros::Timer publish_timer = node_handle.createTimer(ros::Duration(dynamixel_controller.getPublishPeriod()), &HealDMCController::publishCallback, &dynamixel_controller);
